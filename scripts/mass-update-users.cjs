...
async function main() {
  ...
  for (const sub of subjects) {
    for (let i = 1; i <= 3; i++) {
      emails.push(`${sub}_0${i}@fieldforce.test`)
    }
  }

  console.log(`--- Iniciando actualizacion masiva para ${emails.length} usuarios ---`)
  ...
  console.log(`Encontrados ${users.length} usuarios de los ${emails.length} solicitados.`)
